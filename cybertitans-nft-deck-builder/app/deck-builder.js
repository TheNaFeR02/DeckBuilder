...
const synergies = { oceanic: 0, forest: 0, magma: 0, desert: 0, humanoid: 0, electric: 0, arctic: 0, holy: 0, demon: 0, spirit: 0, indomitable: 0, caster: 0, shooter: 0, weaponsMaster: 0, guardian: 0, assassin: 0, shieldmaiden: 0, greedy: 0 }

export default function DeckBuilder({ titans }) {
  const [board, setBoard] = useState(Array(32).fill(null)); // board would be a better name
  const titanCards = titans.map((titan, index) => titanCard(titan.image_url, titan.name, index));
  ...
  function handleDragStart(event) {
    ...
    setDraggedItem(titanCards[active.id]);
  }
  ...
  return (
    <DndContext
      ...
      <div className="h-screen flex gap-10 justify-center flex-col items-center">
        ...
        <Droppable id="_deck">
          <div className="flex flex-wrap gap-3 bg-neutral w-[592px] h-[360px] rounded-md p-3 overflow-y-scroll">
            {deck.map((id, index) => (
              deck[index] !== null ? <div key={index}  >{
                // <DragOverlay>
                  
                  titanCards[deck[index]]
                // </DragOverlay>
